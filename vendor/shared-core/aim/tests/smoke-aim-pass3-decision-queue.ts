...
assert(validDecision.paiSafeStatus === AIM_PAI_SAFE_DECISION_STATUS.SAFE_TO_REVIEW, "Strong documented decision should be SAFE TO REVIEW.");
...
assert(weakDecision.paiSafeStatus === AIM_PAI_SAFE_DECISION_STATUS.REFUSED_INSUFFICIENT_SIGNAL, "Weak evidence should refuse for insufficient signal.");
...
assert(contradictionDecision.paiSafeStatus === AIM_PAI_SAFE_DECISION_STATUS.REFUSED_THESIS_CONTRADICTION, "Contradiction should refuse thesis.");
...
assert(highRiskDecision.paiSafeStatus === AIM_PAI_SAFE_DECISION_STATUS.REFUSED_RISK_TOO_HIGH, "Excessive risk should refuse.");
...
assert(undocumentedDecision.paiSafeStatus === AIM_PAI_SAFE_DECISION_STATUS.REFUSED_UNDOCUMENTED_ACTION, "Undocumented decision should refuse.");
...
assert(prohibitedTradeDecision.paiSafeStatus === AIM_PAI_SAFE_DECISION_STATUS.REFUSED_UNDOCUMENTED_ACTION, "Prohibited action language must be refused.");
...
assert(holdDecision.paiSafeStatus === AIM_PAI_SAFE_DECISION_STATUS.HOLD_FOR_CONFIRMATION, "Moderate evidence should hold for confirmation.");
...
assert(summary[AIM_PAI_SAFE_DECISION_STATUS.SAFE_TO_REVIEW] === 1, "Queue summary should contain one SAFE TO REVIEW.");
assert(summary[AIM_PAI_SAFE_DECISION_STATUS.REFUSED_INSUFFICIENT_SIGNAL] === 1, "Queue summary should contain one insufficient signal refusal.");
assert(summary[AIM_PAI_SAFE_DECISION_STATUS.REFUSED_THESIS_CONTRADICTION] === 1, "Queue summary should contain one thesis contradiction refusal.");
assert(summary[AIM_PAI_SAFE_DECISION_STATUS.REFUSED_RISK_TOO_HIGH] === 1, "Queue summary should contain one risk refusal.");
assert(summary[AIM_PAI_SAFE_DECISION_STATUS.REFUSED_UNDOCUMENTED_ACTION] === 1, "Queue summary should contain one undocumented action refusal.");
