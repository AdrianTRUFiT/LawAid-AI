...
const wallet: EcosystemWalletProfile = {
  walletId: "wallet_or_ref_001",
  ownerId: "owner_or_ref_001",
  homeJurisdiction: "US",
  trustTier: "trusted",
  balances: [
    {
...
const price = buildRealValuePrice({
  merchantId: "merchant_or_ref_001",
  realValueUnits: 10,
  displayCurrency: "USD",
...
if (result.routed || result.decision !== "REFUSED_COMPLIANCE") {
  throw new Error(`Expected REFUSED_COMPLIANCE but received ${result.decision}`);
}

console.log("SMOKE_OPERATOR_ROUTE_REFUSAL=PASS");