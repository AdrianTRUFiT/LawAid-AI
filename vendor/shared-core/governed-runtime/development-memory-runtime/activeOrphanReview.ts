...
export type ActiveOrphanDispositionDecision = {
  memoryId: string;
  decision: "retire_duplicate" | "preserve_standalone";
  approvedBy: string;
  rationale: string[];
  createdAt: string;
};
...
function reviewDir(): string {
  const dir = path.resolve(process.cwd(), "shared-core", "governed-runtime", "store", "active-orphan-review");
  ensureDir(dir);
  return dir;
}
...
function saveDecision(decision: ActiveOrphanDispositionDecision, before: MemoryObject, after: MemoryObject): string {
  const filePath = path.join(
    reviewDir(),
    `${decision.memoryId}__${decision.decision}.active-orphan-review.json`
  );
...
function resolveNext(decision: ActiveOrphanDispositionDecision["decision"]) {
  switch (decision) {
    case "retire_duplicate":
      return { promotion: "retired", verification: "reviewed" };
    case "preserve_standalone":
      return { promotion: "active", verification: "reviewed" };
  }
}

export function reviewActiveOrphan(decision: ActiveOrphanDispositionDecision) {
  const filePath = path.join(memoryDir(), `${decision.memoryId}.memory.json`);

  if (!fs.existsSync(filePath)) {
    throw new Error(`active_orphan_memory_not_found:${decision.memoryId}`);
  }

  const memory = readJsonFile<MemoryObject>(filePath);

  if (memory.promotion !== "active") {
    throw new Error(`active_orphan_review_requires_active_state:${memory.id}:${memory.promotion}`);
  }
